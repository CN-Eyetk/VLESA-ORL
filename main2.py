...
if args_g.pretrained_model_path is not None:
    ...
    GROUP = ("-LIGHT2" if not USE_SATTN else "") + ("-TRANS4" if USE_TRANS else "NoTrans") if USE_EMB_PREP else ((("-TRANS3" if USE_TRANS else "NoTrans") if USE_PREPEND else "-TRANS2") if USE_TRANS else "NoTrans") 
...
def load_arg():
    ...
    args = {"do_train":args_g.do_train,
            "data_path":"neo_converted_dataset",
            ...
            "data_cache_dir":"{}/neo_301_II_{}_{}_{}{}cached".format(root_path,"noprep" if not USE_PREPEND else "prep", "bart_" if BART else "", "emin_" if USE_EMO_IN_DIST else "","w_vad" if args_g.use_vad_labels else ""),
