...
GEN_KWARG_KEYS = ['min_length', 'top_k', 'top_p', 'do_sample', 'pad_token_id', 'max_new_tokens']
MODEL_CARD_TEMPLATE = """---
license: apache-2.0
tags:
- trl
...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        name_unshared_layers: Optional[List[str]] = None,
        ...
        #print(config.accelerator_kwargs)
        ddp_kwargs = DistributedDataParallelKwargs(find_unused_parameters=True)
        config.accelerator_kwargs['kwargs_handlers']=[ddp_kwargs]
        ...
        if isinstance(ref_model, SUPPORTED_ARCHITECTURES):
            ...
            self.ref_model = create_reference_model(self.model, num_shared_layers=num_shared_layers, name_unshared_layers = name_unshared_layers)
        ...
        if self.config.use_warm_up:
            ...
            t_total = len(self.dataloader) * self.config.num_train_epochs
    ...
    def _generate_batched(
        ...
        **generation_kwargs,
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            for generation, mask in zip(generations, (padded_inputs["attention_mask"] if batch_mask is not None else generation_kwargs["attention_mask"])):
                if not self.is_encoder_decoder:
    ...
    def log_stats(
        ...
        if self.accelerator.is_main_process:
            ...
            logs["env/ref_reward_mean"] = np.mean(ref_rewards)
            logs["env/ref_reward_std"] = np.std(ref_rewards)
            #logs["env/ref_reward_dist"] = ref_rewards.cpu().numpy()
